...
export const createActions = (actionsName: Array<string>): any =>
  actionsName.reduce((akk, name) => ({ ...akk, [name]: createAction() }), {})
...
/**
 * Create connect function base on Redux concept to connect React and rxjs without using redux
 * @param selector
 * @param actionSubject
 */
export const connect = (
  ...
  const actions = Object.keys(actionSubject).reduce(
    (akk, key) => ({
      ...akk,
      [key]: (value: any) => actionSubject[key].next(value),
    }),
    {}
  )
...
export class Provide extends Component<Props, State> {
  static propTypes = {
    state$: PropTypes.object.isRequired,
  }
